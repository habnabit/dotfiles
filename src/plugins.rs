...
fn extract_plugins(
    resp: &::capnp::capability::Response<plugin_process::initialize_results::Owned>,
) -> Result<Vec<OwnedMessage<plugin::Owned>>> {
    resp.get()?
        .get_plugins()?
        .iter()
...
fn load_plugin(path: &path::Path) -> BoxFuture<Vec<OwnedMessage<plugin::Owned>>> {
    let child = process::Command::new(path)
        ...
        .spawn_async();
    let ret = future::done(child).map_err(Into::into).and_then(|mut c| {
        ...
        plugin
            ...
            .promise
            .map_err(Into::into)
            .and_then(|resp| future::done(extract_plugins(&resp)))
    });
    Box::new(ret)
}
...
where
    T: version_control_plugin::Server,
{
    let mut ret = OwnedMessage::<plugin::Owned>::init_default();
    {
        ...
        vc.set_vc_name(name);
        vc.set_plugin(
            version_control_plugin::ToClient::new(server).into_client::<::capnp_rpc::Server>(),
        );
    }
    ret
}
...
impl<'a> VcPlugin<'a> {
    pub fn get_vc_name(&self, directory: &str, branch_only: bool) -> BoxFuture<Option<VcStatus>> {
        ...
        .and_then(|(req, vc_name)| req.send().promise.map(move |r| (r, vc_name)))
        .and_then(|(r, vc_name)| {
...
fn select_first_some<T: 'static, A: 'static>(
    (opt, futures): (Option<T>, Vec<A>),
) -> Box<dyn Future<Item = Option<T>, Error = A::Error>>
where
    A: Future<Item = Option<T>>,
{
    match opt {
...
impl PluginLoader {
    ...
    pub fn load_builtin_plugins(mut self) -> Self {
        self.plugins
            .push(builtin_vc_plugin("git", Git(self.handle.clone())));
        self.plugins
            .push(builtin_vc_plugin("hg", Hg(self.handle.clone())));
        self
    }
    ...
    pub fn load_plugins(mut self) -> BoxFuture<Self> {
        ...
        let ret = future::join_all(futures).map(move |plugins| {
